...
PAD_OF_HITTING_THE_BALL = 0


class PongBat:
    def __init__(self, x, y, up_key, down_key, points_x, a=None):
        if a is None:
            self.pygame_rect_obj = pygame.Rect(x, y, PONG_BAT_WIDTH, PONG_BAT_HEIGHT)
        else:
            self.pygame_rect_obj = pygame.Rect(x, y, PONG_BAT_WIDTH, a)
        self.up_key = up_key
...
class PongBall:
    def __init__(self, x, y, image_path=None, movement=None):
        ...
        if movement is None:
            ...
            self.movement_speed_horizontal = movement[0]
            self.movement_speed_vertical = movement[1]
            self.horizontal = movement[2]
            self.vertical = movement[3]
...
def valid_move(new_y):  # returns true if the move is valid else false
...
def get_wanted_y_for_bot(bot_bat, ball):
    """
    :type bot_bat: PongBat
    :type ball: PongBall
    """
    moves = [-1, 1]

    for i in moves:
        if not valid_move(bot_bat.pygame_rect_obj.y + MOVEMENT_SPEED * i):
            moves.remove(i)
            return moves[0]

    current_ball_x, current_ball_y = ball.pygame_rect_obj.x, ball.pygame_rect_obj.y

    dummy_ball = PongBall(current_ball_x, current_ball_y, movement=(ball.movement_speed_horizontal, ball.movement_speed_vertical, ball.horizontal, ball.vertical))
    dummy_ball.hits = ball.hits

    while dummy_ball.pygame_rect_obj.x > bot_bat.pygame_rect_obj.x + PONG_BAT_WIDTH:
        if dummy_ball.pygame_rect_obj.x + PONG_BALL_DIMENSION > WINDOW_WIDTH - PAD_X_OF_BAT - PONG_BAT_WIDTH:
            dummy_ball.ball_hit()

        dummy_ball.check_collision_in_sides()
        dummy_ball.move_ball()

    wanted_y = dummy_ball.pygame_rect_obj.y - PAD_OF_HITTING_THE_BALL

    if wanted_y < 0:
        wanted_y = 0

    if wanted_y > WINDOW_HEIGHT - PONG_BAT_HEIGHT:
        wanted_y = WINDOW_HEIGHT - PONG_BAT_HEIGHT
...
def main():
    ...
    while not to_exit:
        ...
        previous_y = pong_bat_left.pygame_rect_obj.y

        if not no_ball:
            if not found_move:
                wanted_y = get_wanted_y_for_bot(pong_bat_left, pong_ball)
                changeable = True

            if previous_y <= wanted_y <= pong_bat_left.pygame_rect_obj.y + PAD_OF_HITTING_THE_BALL and changeable:
                move = 0
                pong_bat_left.pygame_rect_obj.y = previous_y

            elif pong_bat_left.pygame_rect_obj.y < wanted_y and changeable:
                move = 1

            elif pong_bat_left.pygame_rect_obj.y > wanted_y and changeable:
                move = -1

            if valid_move(pong_bat_left.pygame_rect_obj.y + MOVEMENT_SPEED * move) and valid_move(pong_bat_left.pygame_rect_obj.y + MOVEMENT_SPEED * move + PONG_BAT_HEIGHT):
                pong_bat_left.pygame_rect_obj.y += MOVEMENT_SPEED * move
            else:
                move = 0
                changeable = False

        output = pong_bat_left.check_collision_with_ball(pong_ball)
        pong_bat_right.check_collision_with_ball(pong_ball)

        found_move = output is None and not no_ball
